...
expName = 'Concurrent variable-fixed avoidance schedule'  # from the Builder filename that created this script
...
COD = 5
schedule_list = ['ConcChFR20VI5/FR20FI5', 'ConcChFR20FI5/FR20VI5'] #where random schedule is located
interval = 5
ratio = 20
link_time = 100
initial = False
...
data = [] #array for data
data.append(['time', 'R1', 'n1', 'Cs1', 'R2', 'n2', 'Cs2', 'R3', 'Cs3', 'score', 'phase', 'schedule']) #column names in csv file
...
while global_time.getTime() < session * 60 or score > 0:
    ...
    if n1 >= ratio or n2 >= ratio:
        ...
        if interval_timer.getTime() < 0:
            Cs3 += 1
            score = score - 1
            interval_timer = CountdownTimer(0.2)
    ...
    if global_time.getTime() < 25.0:
        text.text = u'Кликайте на прямоугольники, чтобы не потерять очки'
        text.pos = (0, + 0.5)
        text.draw()
    
    #record data
    tempArray = [global_time.getTime(), R1, n1, Cs1, R2, n2, Cs2, R3, Cs3, score, phase, schedule]
